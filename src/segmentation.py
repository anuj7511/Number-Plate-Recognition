...
def segment_characters(image, plate_name, clahe_clipLimit=3, clahe_tileGridSize=8, erosion_iters=1, dilation_iters=2, display_verbose=1):
    ...
    img_binary_lp = cv2.threshold(img_gray_lp, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)[1]
...
def find_contours(dimensions, plate_name, img, display_verbose=1) :
    ...
    cntrs, _ = cv2.findContours(img.copy(),cv2.RETR_CCOMP+cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
    ...
    for intX, intY, intWidth, intHeight in coords:
        if intWidth > lower_width and intWidth < upper_width and intHeight > lower_height and intHeight < upper_height and intX>prev_x:
    ...
    for intX, intY, intWidth, intHeight in coords :
        ...
        if (lower_width < intWidth < upper_width) and (lower_height < intHeight < upper_height) and intX>prev_x and (lower_Y < intY < upper_Y):
            prev_x = intX + (3*intWidth)/4
            ...
            if intWidth>meanWidth*1.5:
                ...
                char = cv2.copyMakeBorder(char, border_y, border_y, border_x, border_x, cv2.BORDER_CONSTANT, value=255.)
                char = cv2.resize(char, (24, 24))
                # small padding to center the contours
                char = cv2.copyMakeBorder(char, 2, 2, 2, 2, cv2.BORDER_CONSTANT, value=255.)

                cv2.rectangle(ii, (intX,intY), (intWidth//2+intX, intY+intHeight), (50,21,200), 2)
                ...
                x_cntr_list.append(intX+intWidth//2) #stores the x coordinate of the character's contour, to used later for indexing the contours
                char = img[intY:intY+intHeight, intX+intWidth//2:intX+intWidth]
                ...
                cv2.rectangle(ii, (intX+intWidth//2,intY), (intWidth+intX, intY+intHeight), (50,21,200), 2)
                if display_verbose>=1:
                    '''show bounding rectangles for each identified character'''
                    plt.imshow(ii, cmap=plt.cm.binary)
