...
class ReadPlate:
    ...
    def __init__(
        ...
        self.state_codes = [
            'AN', 'AP','AR','AS', 'BR', 'CH', 'PB', 'CG', 'DD', 'DL', 'GA', 'GJ', 'HR', 'HP', 'JK', 'JH', 
            'KL', 'LA', 'LD', 'MP', 'MH', 'MN', 'ML', 'MZ', 'NL', 'OD', 'PY', 'RJ', 'SK', 'TN', 'TS', 'TR', 
            'UP', 'UK', 'WB',
        ]
    ...
    def predict_char(self, char_list):
        ...
        if 9 <= len(fixed_imgs) <= 11:
            state_code = [self.labels_alphabets[self.alphabets_model.predict(img).argmax(-1)[0]] for img in fixed_imgs[:2]]
            last_four = [self.labels_digits[self.digits_model.predict(img).argmax(-1)[0]] for img in fixed_imgs[-4:]]
            ...
            if ("".join(state_code)=='DL') or ("".join(state_code) not in self.state_codes):
                for img in remaining_images[:1]:
                    remaining_preds.append(self.labels_digits[self.digits_model.predict(img).argmax(-1)[0]])
                for img in remaining_images[1:-1]:
                    remaining_preds.append(self.labels36[self.all36_model.predict(img).argmax(-1)[0]])
                for img in remaining_images[-1:]:
                    remaining_preds.append(self.labels_alphabets[self.alphabets_model.predict(img).argmax(-1)[0]])
            else:
    ...
    def fix_dimension(img):
        ...
        assert img.shape==(28,28), f'Input Dimensions expected (28,28) got {img.shape}'
